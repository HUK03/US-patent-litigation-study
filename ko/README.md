...
## GitHub Pages로 배포해서 보기

로컬 HTML 파일을 직접 여는 방식보다, GitHub Pages로 배포하면 팀 내 공유가 더 쉽습니다.

- 워크플로 파일: `.github/workflows/deploy-ko-pages.yml`
- 동작: `ko/` 문서 또는 `scripts/export_html.py` 변경 시 자동 실행
- 산출물: `ko/` 문서를 정적 HTML(`_site/`)로 생성 후 GitHub Pages로 배포

### 저장소에서 1회 설정

1. GitHub 저장소 **Settings → Pages**로 이동
2. **Source**를 **GitHub Actions**로 선택
3. `main` 브랜치에 변경 푸시 후, Actions 탭에서 `Deploy Korean Docs to GitHub Pages` 성공 여부 확인
...
출력 후 `site-ko/index.html`에서 전체 문서 목록으로 진입할 수 있습니다.
